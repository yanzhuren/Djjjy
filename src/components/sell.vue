<template>
  <div class="buy-wrap">
    <div class="buy-session1">
      ...
      <div class="buy-inbox">
        <span>委托价格：</span>
        <input class="buy-price" type="number" v-model="price" :readonly="checked" />
        <div class="buy-check">
    ...
    <div class="buy-session3">
      <div class="buy-nav">
        <router-link
          :to="{path:'/' + $store.state.url1 + '/sell/bshold'}"
          active-class="act-class"
        >持仓</router-link>
        <router-link
          :to="{path:'/' + $store.state.url1 + '/sell/bsentrust'}"
          active-class="act-class"
        >委托</router-link>
        <router-link
          :to="{path:'/' + $store.state.url1 + '/sell/bsdeal'}"
          active-class="act-class"
        >成交</router-link>
        <router-link
          :to="{path:'/' + $store.state.url1 + '/sell/bscapital'}"
          active-class="act-class"
        >资金</router-link>
...
export default {
  ...
  methods: {
    ...
    openInterval() {
      ...
      that.getAllnum(that.$store.state.activestock);
          ...
          if (res.data.code == 1) {
            if (res.data.data.length == 0) {
              ...
              for (let i = 0; i < res.data.data.length; i++) {
                if (res.data.data[i].short == short) {
                  that.allnum = res.data.data[i].surplus;
                  break;
                } else {
                  that.allnum = "";
                }
